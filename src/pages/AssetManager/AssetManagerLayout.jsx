...
export default function AssetManagerLayout() {
  ...
  const myNotifications = (notifications || []).filter(
    n => n.recipientRole === 'Asset Manager' || n.recipientRole === 'All'
  );
  const unreadCount = myNotifications.filter(n => !n.read).length;
  const pendingAllocations = (allocationRequests || []).filter(
    r => r.status === 'Pending Asset Manager'
  ).length;
  ...
  const menuItems = [
    { path: '/asset-manager/dashboard', icon: LayoutDashboard, label: 'Dashboard' },
    { path: '/asset-manager/assets', icon: Database, label: 'Asset Directory' },
    { path: '/asset-manager/allocations', icon: ArrowRightLeft, label: 'Allocations' },
    { path: '/asset-manager/transfers', icon: ArrowRightLeft, label: 'Transfers' },
    { path: '/asset-manager/maintenance', icon: Wrench, label: 'Maintenance' },
    { path: '/asset-manager/audit', icon: SearchCheck, label: 'Audit' },
    { path: '/asset-manager/reports', icon: FileBarChart, label: 'Reports' },
  ];
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex overflow-hidden">
      ...
      >
        <div className="p-6 border-b border-slate-800/60">
          ...
          <button onClick={() => navigate('/asset-manager/profile')} className="w-full text-left bg-slate-900/50 p-4 rounded-xl border border-slate-800/50 hover:border-violet-400/50 transition-colors">
          ...
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {item.label === 'Allocations' && pendingAllocations > 0 && (
      ...
      <div className="flex-1 flex flex-col min-w-0 z-10 relative">
        <header className="h-20 bg-slate-950/50 backdrop-blur-md border-b border-slate-800/60 flex items-center justify-between px-8 z-30">
          <div>
          ...
          <div className="flex items-center gap-6">
            <div className="relative hidden md:block">
              ...
              <input
                type="text"
                placeholder="Search..."
                className="bg-slate-900/50 border border-slate-800 rounded-full pl-10 pr-4 py-2 text-sm text-slate-200 focus:outline-none focus:border-blue-500 w-64 transition-all"
              />
            </div>
                    ...
                    <div className="max-h-[300px] overflow-y-auto">
                      {myNotifications.length === 0 ? (
                        ...
                        myNotifications.map(notif => (
                          <div
                            key={notif.id}
                            className={`p-4 border-b border-slate-800/50 last:border-0 hover:bg-slate-800/30 transition-colors ${!notif.read ? 'bg-slate-800/10' : ''}`}
                          >

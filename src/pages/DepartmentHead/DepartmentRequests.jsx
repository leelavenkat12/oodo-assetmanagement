...
export default function DepartmentRequests() {
  ...
  const handleApprove = (req) => {
    ...
    toast.success(`Request ${req.id} approved and forwarded to Asset Manager.`);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="bg-slate-900/50 backdrop-blur border border-slate-800 rounded-2xl overflow-hidden shadow-xl p-6">
        <h3 className="text-xl font-bold text-slate-100 mb-6">Allocation Requests</h3>
      ...
      <div className="bg-slate-900/50 backdrop-blur border border-slate-800 rounded-2xl overflow-hidden shadow-xl p-6">
        <h3 className="text-xl font-bold text-slate-100 mb-6">Transfer Requests</h3>
        <p className="text-slate-400">No pending transfer requests in your department.</p>
      </div>
    </div>
  );
}
